...
# 3. إدارة البيانات - الترتيب التصاعدي (التدريجي للأمام)
if 'step' not in st.session_state: st.session_state.step = 0

# الفصول مرتبة زمنياً: من 2024 لـ 2025
pdf_data = [
...
# الأسابيع مرتبة زمنياً: مارس أولاً ثم أبريل
device_data = [
...
with col_left:
    ...
    with sub_c1:
        st.markdown(f'<div class="census-box-mini">{m_bars}<div style="position:relative; z-index:10;"><div style="color:#555; font-size:12px; font-weight:bold;">CURRENT CENSUS</div><div style="color:#FFD700; font-size:38px; font-weight:900;">{cur_dev["census"]}</div></div></div>', unsafe_allow_html=True)
    with sub_c2:
        st.markdown(f'<div class="census-box-mini" style="border-color:#00d4ff;">{m_bars}<div style="position:relative; z-index:10;"><div style="color:#555; font-size:12px; font-weight:bold;">OCCUPANCY RATE</div><div style="color:#00d4ff; font-size:38px; font-weight:900;">{cur_dev["occ"]}</div></div></div>', unsafe_allow_html=True)
    ...
    for i, (name, mx, steps) in enumerate(dev_info):
        with g_cols[i]:
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge = {'axis': {'range': [None, mx], 'tickvals': []}, 'bar': {'color': "#222"}, 'bgcolor': "rgba(0,0,0,0)",
